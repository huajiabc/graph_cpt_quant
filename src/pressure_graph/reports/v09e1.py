...
def build_download_manifest(targets: pd.DataFrame, *, max_files: int | None = None) -> pd.DataFrame:
    if targets.empty:
        return pd.DataFrame()
    manifest = (
        targets.groupby(["symbol", "date"], as_index=False, sort=True)
        .agg(
...
def query_bybit_orderbook_file(symbol: str, date: str) -> dict[str, object]:
    ...
    if not rows:
        return {
            "symbol": symbol,
            "date": date,
            "download_status": "missing_remote_file",
            "url": "",
            "filename": "",
            "size": np.nan,
        }
...
def enrich_download_manifest(manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in manifest.itertuples(index=False):
        ...
        try:
            ...
            payload.update(
                {
                    "download_status": "list_files_error",
                    "url": "",
                    "filename": "",
                    "size": np.nan,
                    "error": str(exc),
...
def run_historical_orderbook_replay(cfg: V09E1Config = V09E1Config()) -> dict[str, Path]:
    ...
    targets = load_replay_targets(cfg.source_root, cfg.conflict_root, pool=cfg.pool, max_positions=cfg.max_positions)
    manifest = build_download_manifest(targets, max_files=cfg.max_files)
    ...
    parse_status = pd.concat(status_frames, ignore_index=True) if status_frames else pd.DataFrame()
    ...
    _write_notes(outputs["candidate_notes"], targets, manifest, features, parse_status)
